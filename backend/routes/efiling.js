...
const fileFilter = (req, file, cb) => {
    ...
    if (allowedTypes.includes(file.mimetype)) {
        ...
        cb(new Error('Invalid file type. Allowed: PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX, JPG, PNG, GIF, TXT, ZIP, RAR'), false);
    }
};
...
router.post('/send', protect, upload.single('file'), async (req, res) => {
    try {
        ...
        if (recipientId === req.user._id.toString()) {
            fs.unlinkSync(req.file.path);
            return res.status(400).json({ success: false, message: 'Cannot send file to yourself' });
        }
...
router.get('/inbox', protect, async (req, res) => {
    try {
        ...
        const total = await FileTransfer.countDocuments({ recipient: req.user._id });
        const unreadCount = await FileTransfer.countDocuments({ recipient: req.user._id, isRead: false });
...
router.get('/sent', protect, async (req, res) => {
    try {
        ...
        const total = await FileTransfer.countDocuments({ sender: req.user._id });
...
router.delete('/:id', protect, async (req, res) => {
    try {
        ...
        if (hoursSinceSent > 24) {
            return res.status(400).json({ success: false, message: 'Cannot delete files older than 24 hours' });
        }
