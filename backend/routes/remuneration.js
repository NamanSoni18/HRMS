...
const PayslipRelease = require("../models/PayslipRelease");
...
router.get("/attendance-summary", protect, isManagement, async (req, res) => {
  try {
    ...
    const employeeSummaries = users.map((user) => {
      ...
      if (
        joiningDate &&
        joiningDate.getUTCFullYear() === yearNum &&
...
// @route   POST /api/remuneration/payslip/release
// @desc    Release payslip for a specific month/year to all employees
// @access  Management only (Accountant)
router.post("/payslip/release", protect, isManagement, async (req, res) => {
  try {
    ...
    let payslipRelease = await PayslipRelease.findOne({
      month: monthNum,
      year: yearNum,
    });

    if (payslipRelease && payslipRelease.isReleased) {
      return res.status(400).json({
        success: false,
        message: "Payslip for this month has already been released",
      });
    }

    // Create or update the release record
    if (payslipRelease) {
      payslipRelease.isReleased = true;
      payslipRelease.releasedBy = req.user._id;
      payslipRelease.releasedAt = new Date();
      await payslipRelease.save();
    } else {
      payslipRelease = await PayslipRelease.create({
        month: monthNum,
        year: yearNum,
        isReleased: true,
    ...
    res.json({
      success: true,
      message: `Payslip for ${monthNum}/${yearNum} has been released to all employees`,
      data: payslipRelease,
    });
  } catch (error) {
    console.error("Payslip release error:", error);
    res.status(500).json({
      success: false,
...
// @route   GET /api/remuneration/payslip/status
// @desc    Check if payslip is released for a specific month/year
// @access  Protected (All users)
router.get("/payslip/status", protect, async (req, res) => {
  try {
    const { month, year } = req.query;
    ...
    const payslipRelease = await PayslipRelease.findOne({
      month: monthNum,
      year: yearNum,
    }).populate("releasedBy", "username profile.firstName profile.lastName");

    if (!payslipRelease) {
      return res.json({
        success: true,
        isReleased: false,
        message: "Payslip not yet released",
      });
    }

    res.json({
      success: true,
      isReleased: payslipRelease.isReleased,
      releasedAt: payslipRelease.releasedAt,
      releasedBy: payslipRelease.releasedBy
        ? `${payslipRelease.releasedBy.profile?.firstName || ""} ${
            payslipRelease.releasedBy.profile?.lastName || ""
          }`.trim() || payslipRelease.releasedBy.username
        : null,
    });
  } catch (error) {
    console.error("Payslip status check error:", error);
    res.status(500).json({
      success: false,
      message: "Server error",
      error: error.message,
...
// @route   GET /api/remuneration/payslip/all-releases
// @desc    Get all released payslips
// @access  Management only
router.get("/payslip/all-releases", protect, isManagement, async (req, res) => {
  try {
    const releases = await PayslipRelease.find({ isReleased: true })
      .populate("releasedBy", "username profile.firstName profile.lastName")
      .sort({ year: -1, month: -1 });
